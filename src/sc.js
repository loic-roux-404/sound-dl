...
const LIKES = 'likes'
const simplifiedSong = ({ permalink_url, title }) => ({ uri: permalink_url, title })
...
const getSongsInPlaylist = async (uri) => {
    return (await client.getPlaylist(uri))[TRACKS]
        .map(simplifiedSong)
}

const downloadSongs = (folder, songsUri, count = null) => {
    if (count) songsUri = songsUri.slice(0, count)
    console.log(songsUri)
    songsUri.every(({ title, uri }) => {
        console.log(uri)
        return client.getSongInfo(uri, {})
            .then(async song => {
...
const downloadPlaylistSongs = async ({ folder, uri, count }) => {
    downloadSongs(folder, await getSongsInPlaylist(uri), count)
}
...
`)
    ...
    if (playlistsEnabled) playlists({ folder, uri: subject, count })
